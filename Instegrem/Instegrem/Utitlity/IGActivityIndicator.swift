...
class IGActivityIndicator: CAGradientLayer {
    ...
    init(frame: CGRect, addWidthAndHeight: Float, startPoint: CGPoint = CGPoint(x: 0.0, y: 1.0),
                    endPoint: CGPoint = CGPoint(x: 1.0, y: 0.0),
                    location: [NSNumber] = [0, 0.4, 1],
        ...
        self.frame = CGRect(x: CGFloat(-addWidthAndHeight) + frame.minX, y: CGFloat(-addWidthAndHeight) + frame.minY, width: frame.width + CGFloat(addWidthAndHeight)*2, height: frame.height + CGFloat(addWidthAndHeight)*2)
        settingSegment()
    }
